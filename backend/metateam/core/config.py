...
# backend/metateam/core/config.py
# parents[0]=core, [1]=metateam, [2]=backend, [3]=repo
BACKEND_ROOT = Path(__file__).resolve().parents[2]
REPO_ROOT = Path(__file__).resolve().parents[3]
load_dotenv(REPO_ROOT / ".env")
load_dotenv(BACKEND_ROOT / ".env")

# Back-compat alias used across services (points at backend/)
ROOT = BACKEND_ROOT
...
class Settings:
    ...
    skills_dir: Path = field(default_factory=lambda: BACKEND_ROOT / "skills")
    memory_file: Path = field(default_factory=lambda: BACKEND_ROOT / "memory" / "MEMORY.md")
    sessions_dir: Path = field(default_factory=lambda: BACKEND_ROOT / "sessions")
